...
def test_fast_flood_semreduce_patch_only_dynamic_regions() -> None:
    torch.manual_seed(31)
    patch_tokens = torch.randn(16, 12)
    classifier = torch.randn(9, 12)
    ...
    assert result.soft_positions.shape == (result.patch_tokens.shape[0], 2)


def test_fast_flood_semreduce_sequence_keeps_cls() -> None:
    torch.manual_seed(37)
    sequence = torch.randn(1, 10, 16)
    classifier = torch.randn(12, 16)
    reducer = FastFloodSemReduce(FastFloodSemReduceConfig(candidate_classes=5))
...
def test_fast_flood_semreduce_merges_uniform_grid_component() -> None:
    patch_tokens = torch.ones(4, 8)
    classifier = torch.randn(10, 8)
    ...
    assert result.patch_tokens.shape == (1, 8)
    assert torch.equal(result.masses, torch.tensor([4]))
    assert torch.equal(result.assignments, torch.zeros(4, dtype=torch.long))
